    ...
    <>
        ...
        >
          <Button asChild variant="ghost" className="flex items-center gap-2 px-2 py-1 rounded-full hover:bg-zinc-800/60 mr-1 h-auto">
            <Link to="/" onClick={handleLogoClick}>
              ...
              >
                <span className="text-[10px] font-bold" style={{ color: PRIMARY }}>C</span>
              </div>
              <span className="text-zinc-200 text-sm font-semibold tracking-tight hidden sm:block">Curatist</span>
            </Link>
          </Button>
          ...
          <div className="hidden md:flex items-center ml-1 shrink-0">
            <Button asChild size="xs" className="rounded-full font-semibold text-white" style={{ background: PRIMARY }}>
              <a href="https://github.com/WebchemistCorp/curatist/releases" target="_blank" rel="noreferrer">
                다운로드
              </a>
            </Button>
          </div>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-40 bg-zinc-950/95 backdrop-blur-3xl flex flex-col items-center justify-center gap-4 md:hidden">
          ...
          <a
            href="https://github.com/WebchemistCorp/curatist/releases"
            target="_blank"
            rel="noreferrer"
            style={{ animationDelay: `${NAV_ITEMS.length * 60}ms`, background: PRIMARY } as React.CSSProperties}
            className="reveal mt-4 px-8 py-3 rounded-full text-white font-semibold transition-all duration-300"
          >
            다운로드
          </a>
        </div>
