...
pub fn run() -> Result<()> {
    ...
    println!("{} cur_mails, cur_schedules, cur_spending 테이블 적용 완료", style("✓").green());

    create_storage_bucket()?;

    println!(
        "{} curatist setup 완료 — `curatist mail add` 등 커맨드를 사용할 수 있습니다.",
        style("✓").green().bold()
    );
    Ok(())
}
    ...
    doc.get("supabase")
        ...
        .ok_or_else(|| {
            anyhow::anyhow!(
             ...
             \n\
             curatist config set database-url \"postgresql://postgres.[ref]:[pw]@...\""
            )
        })
}
